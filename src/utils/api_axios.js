...
import CountryMeteoData from '../data/CountryMeteoData.json';
...
async function getAllWeatherData() {
  ...
  for (const countryData of CountryMeteoIndex) {
    ...
    const weatherData = CountryMeteoData;
    if (weatherData && weatherData[i].data) {
      // Calculer les moyennes mensuelles de tmax
      const monthlyAvgTmax = calculateMonthlyTmaxAverage(weatherData[i].data.data);

      // Calculer les moyennes mensuelles de précipitations (prcp)
      const monthlyAvgPrcp = calculateMonthlyPrcpAverage(weatherData[i].data.data);
